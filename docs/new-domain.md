...
```bash
pip install "wss @ git+https://github.com/<owner>/wss-engine.git@v<version>"
wss init ../wss-arxiv --owner <owner> --title "arXiv Listing History"
```
...
`wss validate` passes on the fresh scaffold, and nothing captures
...
4. `wss doctor <source_id>` — **read the raw response** before
   trusting any field name.
5. Flip `status: active`, commit, push under the same owner as the engine,
   set the `WSS_CONTACT` secret, and dispatch `capture-daily` once by
   hand.
...
- **The engine pin is stamped at generation time.** Workflows install
  `wss@v<the version that scaffolded the repo>`, so a repo's
...
```bash
wss init ../wss-template --owner <owner> --title "Template"
