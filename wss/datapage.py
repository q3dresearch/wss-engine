...
def build(root: Path | str, repo_url: str = "") -> str:
    ...
    # The prose must not read "offered under ;" when nothing is declared. An
    # absent licence is a statement in itself and the sentence has to say so.
    licence_sentence = (
        f"This dataset is offered under {e(licence)}."
        if licence else
        "This dataset offers no onward licence: the capture adds a timestamp and "
        "a parse, neither of which creates a right over the underlying values."
    )
    figures_html = ""
    if figures:
        cards = "\n".join(
...
<footer>
<p>Repository code and captured data are licensed separately.
{licence_sentence} Each publisher's own terms govern their material and are
listed per source above. Generated by <code>wss datapage</code> — do not edit
by hand.</p>
</footer>
</body>
</html>
